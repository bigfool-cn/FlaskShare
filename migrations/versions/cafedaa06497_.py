...
Revision ID: cafedaa06497
Revises: 
Create Date: 2018-02-04 11:37:25.452973
...
def upgrade():
    ...
    sa.Column('beuser_id', sa.Integer(), nullable=True),
    sa.Column('addtime', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['beuser_id'], ['user.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_attention_addtime'), 'attention', ['addtime'], unique=False)
    op.create_table('resource',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('title', sa.String(length=255), nullable=True),
    sa.Column('url', sa.String(length=255), nullable=True),
    sa.Column('info', sa.Text(), nullable=True),
    sa.Column('downloadnum', sa.BigInteger(), nullable=True),
    ...
    op.create_index(op.f('ix_resource_addtime'), 'resource', ['addtime'], unique=False)
    ...
    op.create_index(op.f('ix_useroplog_addtime'), 'useroplog', ['addtime'], unique=False)
    ...
    op.create_index(op.f('ix_adminoplog_addtime'), 'adminoplog', ['addtime'], unique=False)
    ...
    op.create_index(op.f('ix_resourcecol_addtime'), 'resourcecol', ['addtime'], unique=False)
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index(op.f('ix_resourcecol_addtime'), table_name='resourcecol')
    op.drop_table('resourcecol')
    op.drop_index(op.f('ix_comment_addtime'), table_name='comment')
    op.drop_table('comment')
    op.drop_index(op.f('ix_adminoplog_addtime'), table_name='adminoplog')
    op.drop_table('adminoplog')
    op.drop_index(op.f('ix_adminlog_addtime'), table_name='adminlog')
    op.drop_table('adminlog')
    op.drop_index(op.f('ix_useroplog_addtime'), table_name='useroplog')
    op.drop_table('useroplog')
    op.drop_index(op.f('ix_userlog_addtime'), table_name='userlog')
    op.drop_table('userlog')
    op.drop_index(op.f('ix_resource_addtime'), table_name='resource')
    op.drop_table('resource')
    op.drop_index(op.f('ix_attention_addtime'), table_name='attention')
